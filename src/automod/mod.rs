...
pub async fn preflight(
    ...
    if count + files.len() as i64 > state.automod.max_held
        || size + files.iter().map(|f| f.2.len() as i64).sum::<i64>() > state.automod.max_held_bytes
    {
    ...
    for (_, _, bytes) in files {
        ...
        for rule in &policy.rules {
            if matches!(rule.action, Action::Reject)
                && policy::non_media_match(state, rule, channel, &auth.user_id, &digest).await?
            {
                record_event(
                    state,
                    None,
                    channel.space_id.as_deref().unwrap_or("*"),
                    None,
                    "reject",
                    serde_json::json!({"rule_id":rule.id,"author_id":auth.user_id,"hash":digest}),
                )
                .await?;
...
pub async fn enqueue(
    ...
    let outcome = async {
        ...
        for (filename,content_type,bytes) in files {
            ...
            tokio::fs::write(private_path(state,&id), bytes).await.map_err(io)?;
            sqlx::query(&db::q("INSERT INTO automod_uploads (id,message_id,channel_id,space_id,author_id,filename,content_type,size,hash,status,created_at,expires_at,next_attempt) VALUES (?,?,?,?,?,?,?,?,?,'pending',?,?,?)"))
                .bind(&id).bind(message).bind(&channel.id).bind(&channel.space_id).bind(author).bind(filename).bind(content_type).bind(bytes.len() as i64).bind(hash(bytes)).bind(now()).bind(now()+i64::from(policy.retention_days)*86400).bind(now()).execute(&mut *tx).await?;
        }
        tx.commit().await?;
        Ok::<_,AppError>(())
    }.await;
...
pub async fn record_event(
    ...
    sqlx::query(&db::q("INSERT INTO automod_events (id,upload_id,scope_id,actor_id,action,details,created_at) VALUES (?,?,?,?,?,?,?)"))
        .bind(crate::snowflake::generate()).bind(upload).bind(scope).bind(actor).bind(action).bind(details.to_string()).bind(now()).execute(&state.db).await?;
    Ok(())
}
async fn scan(state: &AppState, upload: &Upload) -> Result<ScanResult, String> {
    ...
    let result = tokio::time::timeout(state.automod.scan_timeout, async {
        if let Some(container) = video::container(&bytes) {
            ...
            {
                return Err("video decoder did not return five samples".to_string());
            }
    ...
    sqlx::query(&db::q("INSERT INTO automod_cache (hash,scanner_version,result,expires_at) VALUES (?,?,?,?) ON CONFLICT(hash,scanner_version) DO UPDATE SET result=excluded.result, expires_at=excluded.expires_at"))
        .bind(&upload.hash).bind(&version).bind(serde_json::to_string(&result).map_err(|e|e.to_string())?).bind(now()+86400).execute(&state.db).await.map_err(|e|e.to_string())?;
    Ok(result)
}

/// One bounded worker per server; retries and held uploads survive restarts.
/// Policy updates and reviews share this lock to avoid overriding a decision
/// made while inference was in flight. This server uses a single writer process.
pub async fn process_one(state: &AppState) -> Result<bool, AppError> {
    ...
    if permissions::require_channel_permission(&state.db, &channel.id, &author, "send_messages")
        .await
        .is_err()
        || (!channel.space_id.as_deref().unwrap_or("").is_empty()
            && permissions::require_not_timed_out(
                &state.db,
                channel.space_id.as_deref().unwrap(),
                &author,
            )
            .await
            .is_err())
    {
        finish(
            ...
            &upload,
            "quarantined",
            "uploader no longer has permission to post",
            None,
            None,
            None,
        )
    ...
    for rule in &policy.rules {
        ...
        } = &rule.trigger
        {
                ...
                match scan(state, &upload).await {
                    ...
                    Err(e) => {
                        sqlx::query(&db::q("UPDATE automod_uploads SET reason = ?, attempts = attempts + 1, next_attempt = ? WHERE id = ? AND status = 'pending'"))
                            .bind(&e).bind(now()+60).bind(&upload.id).execute(&state.db).await?;
                        if upload.reason != e {
                            record_event(
                                state,
                                Some(&upload.id),
            ...
            policy::non_media_match(state, rule, &channel, &upload.author_id, &upload.hash).await?
        };
        if fires {
            matched = Some(rule);
            break;
...
pub async fn finish(
                ...
                if let Ok(format) = image::guess_format(&bytes) {
                    if let Ok((width, height)) =
                        image::ImageReader::with_format(std::io::Cursor::new(&bytes), format)
                            .into_dimensions()
                    {
                        dimensions = (Some(i64::from(width)), Some(i64::from(height)));
                    }
                }
    ...
    if let Some(space) = &upload.space_id {
        ...
        sqlx::query(&db::q("INSERT INTO audit_log (id,space_id,user_id,action_type,target_id,target_type,reason,changes) SELECT ?,id,?,?,?,'attachment',?,? FROM spaces WHERE id=?"))
            .bind(crate::snowflake::generate()).bind(&actor).bind(format!("automod.{status}")).bind(&upload.id).bind(&reason).bind(details.to_string()).bind(space).execute(&mut *tx).await?;
    }
    if let Some(rule) = rule {
        if matches!(rule.action, Action::Flag) && status == "published" {
            sqlx::query(&db::q("INSERT INTO reports (id,space_id,reporter_id,target_type,target_id,channel_id,category,description) VALUES (?,?,?,'message',?,?,'nsfw',?)"))
                .bind(crate::snowflake::generate()).bind(&upload.space_id).bind(&actor).bind(&upload.message_id).bind(&upload.channel_id).bind(format!("AutoMod rule {}: attachment {}",rule.id,upload.id)).execute(&mut *tx).await?;
        }
    ...
    if status == "published" || upload.status == "published" {
        if let Ok(message) = db::messages::get_message_row(&state.db, &upload.message_id).await {
            ...
            broadcast(state, upload, "message.update", data, "messages", None).await;
        }
    }
...
pub async fn run(state: AppState) {
    ...
    let mut last_orphan_scan = tokio::time::Instant::now() - Duration::from_secs(3600);
    loop {
        interval.tick().await;
        if last_orphan_scan.elapsed() >= Duration::from_secs(3600) {
            if let Err(e) = reconcile_orphans(&state).await {
                tracing::warn!("automod orphan cleanup: {e}");
            }
            last_orphan_scan = tokio::time::Instant::now();
        }
...
pub async fn cleanup(state: &AppState) -> Result<(), AppError> {
    ...
    sqlx::query(&db::q("DELETE FROM automod_cache WHERE expires_at <= ?"))
        ...
        .execute(&state.db)
        .await?;
    // Bound the cache independently of the upload queue.
    sqlx::query("DELETE FROM automod_cache WHERE (hash,scanner_version) NOT IN (SELECT hash,scanner_version FROM automod_cache ORDER BY expires_at DESC LIMIT 10000)").execute(&state.db).await?;
    Ok(())
}
...
pub async fn reconcile_orphans(state: &AppState) -> Result<(), AppError> {
    ...
    while let Some(entry) = entries.next_entry().await.map_err(io)? {
        ...
        if !meta.is_file()
            || meta.modified().map_err(io)?.elapsed().unwrap_or_default()
